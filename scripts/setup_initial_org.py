...
async def create_superuser_if_not_exists():
    """
    Auto-create superuser from FIRST_SUPERUSER env vars if it doesn't exist.
    Called during app startup for production deployments.
    """
    ...
    async with async_session_maker() as session:
        ...
        existing_user = await _get_user_by(session, email=settings.FIRST_SUPERUSER)
